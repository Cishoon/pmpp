...
__global__ void spmv_jds_kernel(int* colIdx, float* values, int* rowPerm, int* iterPtr,
    ...
    if (tid < numRows) {
        ...
        for (int it = 0; it < numTiles; it++) {
            int i = iterPtr[it] + tid;
            if (i >= iterPtr[it + 1]) break; // 当前行在该迭代层没有非零元素
            int col = colIdx[i];
            float val = values[i];
...
torch::Tensor spmv_jds(torch::Tensor colIdx, torch::Tensor values, torch::Tensor rowPerm, torch::Tensor iterPtr, torch::Tensor x, int numRows, int numTiles) {
    auto y = torch::zeros({numRows}, x.options());
    
    dim3 block(BLOCK_SIZE);
    dim3 grid(cdiv(numRows, BLOCK_SIZE));
    
    spmv_jds_kernel<<<grid, block>>>(
        colIdx.data_ptr<int>(),
        values.data_ptr<float>(),
        rowPerm.data_ptr<int>(),
        iterPtr.data_ptr<int>(),
        x.data_ptr<float>(),
        y.data_ptr<float>(),
        numRows,
        numTiles
