...
__global__ void spmv_coo_kernel(
    int nnz, 
    int* rowIdx, int* colIdx, float* values, 
    float* x, float* y) {
    // TODO
...
/*
x 是 1 行 col 列的
把A的一行与x的col列（一个元素）相乘得到y的col列
*/

torch::Tensor spmv_coo(torch::Tensor rowIdx, torch::Tensor colIdx, 
                       torch::Tensor values, torch::Tensor x, int numRows) {
    auto y = torch::zeros({numRows}, x.options());
    int nnz = rowIdx.size(0);
    
    dim3 block(BLOCK_SIZE);
    dim3 grid(cdiv(nnz, BLOCK_SIZE));
    
    spmv_coo_kernel<<<grid, block>>>(
        nnz, 
        rowIdx.data_ptr<int>(), 
        colIdx.data_ptr<int>(), 
